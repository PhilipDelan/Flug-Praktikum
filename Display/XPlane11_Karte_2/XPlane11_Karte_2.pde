...
InetAddress xplaneAddress;
int xplanePort = 49000; // Default XPlane port
...
class waypoint {
  ...
  void cross(boolean isCurrent) {
    ...
    line(pixel_x, pixel_y - 5, pixel_y + 5, pixel_y);
  }
}
...
void setup() {
  ...
  try {
    ...
    xplaneAddress = InetAddress.getByName("127.0.0.1"); // Replace with the actual IP address of XPlane if different
...
void updateRWSK() {
  if (newWaypointAdded && currentWaypointIndex < wpList.size()) {
    ...
    if (currentWaypointIndex == 0) {                                                                                  // wenn erster Waypoint
      if (distanceWaypoint < THRESHOLD_DISTANCE) {
        currentWaypointIndex++;
      }
...
void sendHeading() {
  ...
  try {
    ...
    DatagramPacket packet = new DatagramPacket(buffer, buffer.length, xplaneAddress, xplanePort);
